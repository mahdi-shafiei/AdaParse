#!/usr/bin/env bash
# Prefetch the Hugging Face AdaParse SciBERT checkpoint into a local directory
#
# Usage:
#   download_adaparse_checkpoints.sh <TARGET_DIR> [REVISION]
#
# Env overrides:
#   REPO_ID            (default: 7shoe/adaparse-scibert-uncased)
#   QUIET=1            (less chatty; hides progress bars)
#   HUGGINGFACE_TOKEN  (optional; inherited by CLI if set)
#
# Notes:
# - Avoids ~/.local/bin/hf; prefers 'huggingface-cli', then 'python -m huggingface_hub.cli.hf'.
# - Forces ONLINE just for this command; resumes partial downloads.
# - Uses a per-target local cache to reduce lock chatter.

set -Eeuo pipefail

REPO_ID="${REPO_ID:-7shoe/adaparse-specter-docwise}"
TARGET_DIR="${1:-}"
REVISION="${2:-main}"
QUIET="${QUIET:-0}"
...
# Local cache + quiet mode niceties
...
need_download() {
  # For BERT-style models: config + (safetensors|bin) + (tokenizer.json|vocab.txt)
  [[ -f "${TARGET_DIR}/config.json" ]] && \
  { [[ -f "${TARGET_DIR}/model.safetensors" ]] || [[ -f "${TARGET_DIR}/pytorch_model.bin" ]]; } && \
  { [[ -f "${TARGET_DIR}/tokenizer.json" ]] || [[ -f "${TARGET_DIR}/vocab.txt" ]]; }
}
...
# Online only for this call; resume partial downloads
